...
class TenantService:
    ...
    def _hostel_to_dict(self, h) -> dict:
        """Normalize hostel ORM object to dict with sensible fallbacks.
 
        Some DB rows use `hostel_name`/`full_address` while alias fields
        `name`/`address` may be NULL. Prefer the alias when present,
        otherwise fall back to the canonical fields.
        """
        return {
            "id": getattr(h, "id", None),
            "name": (getattr(h, "name", None) or getattr(h, "hostel_name", None)),
            "address": (getattr(h, "address", None) or getattr(h, "full_address", None)),
        }
    ...
    def get_user_hostels(self, user_id: int, user_role: str) -> List[dict]:
        ...
        if user_role == Role.SUPERADMIN:
            ...
            return [self._hostel_to_dict(h) for h in hostels]
            ...
            if user and user.hostel_id:
                ...
                if hostel:
                    return [self._hostel_to_dict(hostel)]
            return []
    ...
    def assign_admin_to_hostels(self, admin_id: int, hostel_ids: List[int], assigner_role: str) -> dict:
        ...
        assigned = [self._hostel_to_dict(h) for h in assigned_hostels]
