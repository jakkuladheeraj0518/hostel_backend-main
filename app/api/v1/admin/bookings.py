
from fastapi import APIRouter, Depends, HTTPException
from sqlalchemy.orm import Session
from datetime import datetime
from typing import List

from app.core.database import get_db
from app.models.booking import Booking, BookingStatus
from app.models.rooms import Room
from app.schemas.booking import (
    BookingCreateSchema,
    BookingResponseSchema,
    BookingUpdateSchema,
    BookingStatusUpdateSchema,
)

# RBAC imports
from app.core.roles import Role
from app.core.permissions import Permission
from app.api.deps import role_required, permission_required


router = APIRouter(prefix="/bookings", tags=["Bookings"])


# ---------------------------------------------------------
# CHECK DOUBLE BOOKING
# ---------------------------------------------------------
def is_room_available(db: Session, room_id: int, check_in: datetime, check_out: datetime):
    return (
        db.query(Booking)
        .filter(
            Booking.room_id == room_id,
            Booking.status == BookingStatus.confirmed.value,
            Booking.check_in < check_out,
            Booking.check_out > check_in
        )
        .first()
        is None
    )


# ---------------------------------------------------------
# CREATE BOOKING (Visitor, Student, Admin, Superadmin)
# ---------------------------------------------------------
@router.post("/", response_model=BookingResponseSchema)
def create_booking(
    payload: BookingCreateSchema,
    db: Session = Depends(get_db),
    current_user = Depends(
        role_required([Role.VISITOR, Role.STUDENT, Role.ADMIN, Role.SUPERADMIN])
    ),
    _: None = Depends(permission_required(Permission.CREATE_BOOKING)),
):
    room = db.query(Room).filter(Room.id == payload.room_id).first()
    if not room:
        raise HTTPException(404, "Room not found")

    if room.available_beds <= 0:
        raise HTTPException(400, "No beds available in this room")

    if not is_room_available(db, payload.room_id, payload.check_in, payload.check_out):
        raise HTTPException(400, "Room is not available for these dates")

    booking = Booking(
        hostel_id=payload.hostel_id,
        room_id=payload.room_id,
        visitor_id=payload.visitor_id,
        check_in=payload.check_in,
        check_out=payload.check_out,
        amount_paid=payload.amount_paid,
        status=BookingStatus.pending.value,
    )

    db.add(booking)
    db.commit()
    db.refresh(booking)

    return booking


# ---------------------------------------------------------
# GET ALL BOOKINGS (Admin, Superadmin)
# ---------------------------------------------------------
@router.get("/", response_model=List[BookingResponseSchema])
def list_bookings(
    db: Session = Depends(get_db),
    current_user = Depends(role_required([Role.ADMIN, Role.SUPERADMIN])),
    _: None = Depends(permission_required(Permission.VIEW_BOOKING)),
):
    return db.query(Booking).all()


# ---------------------------------------------------------
# GET SINGLE BOOKING
# Admin, Superadmin → can view all  
# Student → Optional: only own booking (but not implemented here)
# ---------------------------------------------------------
@router.get("/{booking_id}", response_model=BookingResponseSchema)
def get_booking(
    booking_id: int,
    db: Session = Depends(get_db),
    current_user = Depends(role_required([Role.ADMIN, Role.SUPERADMIN])),
    _: None = Depends(permission_required(Permission.VIEW_BOOKING)),
):
    booking = db.query(Booking).filter(Booking.id == booking_id).first()
    if not booking:
        raise HTTPException(404, "Booking not found")
    return booking


# ---------------------------------------------------------
# UPDATE BOOKING (Admin, Superadmin)
# ---------------------------------------------------------
@router.put("/{booking_id}", response_model=BookingResponseSchema)
def update_booking(
    booking_id: int,
    payload: BookingUpdateSchema,
    db: Session = Depends(get_db),
    current_user = Depends(role_required([Role.ADMIN, Role.SUPERADMIN])),
    _: None = Depends(permission_required(Permission.MODIFY_BOOKING)),
):
    booking = db.query(Booking).filter(Booking.id == booking_id).first()
    if not booking:
        raise HTTPException(404, "Booking not found")

    if payload.room_id:
        room = db.query(Room).filter(Room.id == payload.room_id).first()
        if not room:
            raise HTTPException(404, "Room not found")

        if not is_room_available(
            db,
            payload.room_id,
            payload.check_in or booking.check_in,
            payload.check_out or booking.check_out
        ):
            raise HTTPException(400, "New room not available on selected dates")

        booking.room_id = payload.room_id

    if payload.check_in:
        booking.check_in = payload.check_in
    if payload.check_out:
        booking.check_out = payload.check_out

    db.commit()
    db.refresh(booking)
    return booking


# ---------------------------------------------------------
# UPDATE STATUS (Admin, Superadmin)
# ---------------------------------------------------------
@router.patch("/{booking_id}/status", response_model=BookingResponseSchema)
def update_status(
    booking_id: int,
    payload: BookingStatusUpdateSchema,
    db: Session = Depends(get_db),
    current_user = Depends(role_required([Role.ADMIN, Role.SUPERADMIN])),
    _: None = Depends(permission_required(Permission.MODIFY_BOOKING)),
):
    booking = db.query(Booking).filter(Booking.id == booking_id).first()
    if not booking:
        raise HTTPException(404, "Booking not found")

    new_status = payload.status

    # Normalize incoming status (pydantic Enum) to string value for comparisons and DB storage
    new_status_value = new_status.value if hasattr(new_status, "value") else new_status

    if new_status_value == BookingStatus.confirmed.value:
        room = db.query(Room).filter(Room.id == booking.room_id).first()
        if room.available_beds <= 0:
            raise HTTPException(400, "No beds available to confirm booking")
        room.available_beds -= 1

    if new_status_value == BookingStatus.cancelled.value:
        room = db.query(Room).filter(Room.id == booking.room_id).first()
        room.available_beds += 1

    booking.status = new_status_value
    db.commit()
    db.refresh(booking)
    return booking
